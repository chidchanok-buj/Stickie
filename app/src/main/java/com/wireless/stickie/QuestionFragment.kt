...
class QuestionFragment : Fragment(), IAnswerSelect {
    ...
    ): View?
    {
        ...
        if (question != null) {
            progress_bar = itemView.findViewById(R.id.process_bar) as ProgressBar
    ...
    override fun selectedAnswer(): CurrentQuestion {
        ...
        if (Common.answerSheetList[questionIndex].type == Common.ANSWER_TYPE.NO_ANSWER) {
            ...
            if (Common.selected_values.size > 1) {// If multiple choice
                ...
                // Getting the first character of the answer
                // Ex: arr[0] = A. Paris
                //     arr[1] = B. New York
                //     Correct Answer = A, B
                //     This code will get 'A' and 'B' in the array for us
            ...
            if (question != null) {
                ...
                Toast.makeText(activity, "Cannot ge the question", Toast.LENGTH_SHORT).show()
                currentQuestion.type = Common.ANSWER_TYPE.NO_ANSWER
            }
